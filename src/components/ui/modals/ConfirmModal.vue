...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const yesButton = ref<HTMLElement>();
    ...
    const value = ref();
    const question = ref("");
    const yesTitle = ref<TranslateResult>("");
    const noTitle = ref<TranslateResult>("");
    const subtitle = ref<TranslateResult>("");
    const inputString = ref<TranslateResult>("");
    onMounted(() => {
      title.value = $t("shared.confirm").toString();
      closeText.value = $t("shared.close").toString();
      if (props.inputType === "email") {
        inputString.value = store.state.account.user?.email ?? "";
      }
    })
    function show(
      _question: TranslateResult,
      _yesTitle?: TranslateResult,
      _noTitle?: TranslateResult,
      _subtitle?: TranslateResult,
      _inputString?: TranslateResult
    ) {
      question.value = _question.toString();
      showing.value = true;
      if (_yesTitle) {
        yesTitle.value = _yesTitle;
      }
      if (_noTitle) {
        noTitle.value = _noTitle;
      }
      if (_subtitle) {
      ...
      nextTick(() => {
        setTimeout(() => {
          ...
          if (props.inputType === "email" && inputEmail.value) {
      ...
      if (_inputString) {
        inputString.value = _inputString;
      }
    }
    ...
    function yes() {
      if (props.inputType) {
        value.value = inputString.value;
        if (
          props.validateInput &&
          (!inputString.value || inputString.value.toString().trim() === "")
        ) {
          return;
        }
      }
      context.emit("yes", value.value);
