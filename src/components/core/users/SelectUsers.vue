  ...
  setup(_, context) {
    ...
    function reload() {
      ...
      services.tenantUsers
        .getAll()
        .then((response) => {
          items.value = response.map((f) => f.user);
        })
        .catch((error) => {
    ...
    const filteredItems = computed((): UserDto[] => {
      ...
      return items.value.filter((f) => f.id?.toUpperCase().includes(searchInput.value.toUpperCase()) || f.email?.toString().toUpperCase().includes(searchInput.value.toUpperCase()) || f.firstName?.toString().toUpperCase().includes(searchInput.value.toUpperCase()) || f.lastName?.toString().toUpperCase().includes(searchInput.value.toUpperCase()));
    })
